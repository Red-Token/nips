...
At time T=900 ARK Provider sends out a Round Renewal request with a timeout of 50 blocks. During this time, the
participants that want to participate in the next round need to announce their intention to do so. They do this by
creating a new set of VUTXO from the current VUTXO:s.

First ARK Provider announces a new round with a NewRoundAnnouncementEvent.

The user responds to this with a RoundParticipationRequestEvent. 


At time T=950 no further transactions are allowed on the round 1 VUTXO:s, instead the ARK continues on round 2 VUTXO:s.

At this point in time, the users are encouraged to sign the new tree, if this is done, then round 2 is fully active.
If this does not happen for the next 50 blocks, then the round goes into supervisor mode and ARK Provider signs the
transition unilaterally.
